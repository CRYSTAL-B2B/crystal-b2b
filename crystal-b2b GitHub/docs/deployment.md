...
> Актуальный релиз и preview 3011 собираются **из этой папки** (`crystal-b2b GitHub`).
> Корневое VPS-приложение устарело; не использовать его для релиза или предпросмотра.
> Hero avatar интегрирован сюда 2026-09-20; [отчёт](SYSTEM_AVATAR_HERO.md).
...
## Предпросмотр сборки на VPS

Смотреть собранную страницу до деплоя: **http://185.161.70.46:3011**

Раздаёт каталог `out/` — ровно тот артефакт, что уходит в Pages. Работает
через systemd-юнит `crystal-b2b.service` (копия лежит рядом:
[crystal-b2b.service](crystal-b2b.service)), автозапуск включён,
`Restart=always`.

```bash
systemctl status crystal-b2b       # состояние
systemctl restart crystal-b2b      # перезапуск
journalctl -u crystal-b2b -n 50    # логи
```

Сервис отдаёт файлы с диска, поэтому **после правок нужна пересборка** —
перезапускать его не требуется:

```bash
cd "crystal-b2b GitHub"
NEXT_PUBLIC_SITE_URL=https://b2b-system.pro npm run build
```

Переменная задаётся явно, чтобы canonical и JSON-LD совпадали с продом:
в `.env.local` её нет.

Порт открыт на всех интерфейсах, то есть страница доступна всем, кто знает
адрес. От индексации её закрывает заголовок `X-Robots-Tag: noindex` —
он задан в `preview-serve.json`. Держать там незавершённый контент,
который не должен попасть на глаза посторонним, всё равно не стоит.

До 2026-08-20 этот юнит запускал `next start` для домена
